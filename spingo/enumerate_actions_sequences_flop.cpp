...
public:
    ...
    int current_player() const {
        if (game_over || (round != "preflop" && round != "flop"))
            return -1;
        return next_player;
    }
    ...
    bool betting_round_complete() {
        ...
        // Special case for flop: Check if player 1 has posted the big blind and others have acted
        if (round == "flop") {
            ...
            // If all pots are equal and all players have acted, the round is complete
...
int main() {
    // Use BFS to enumerate all possible action sequences in the flop round
    queue<SimplifiedSpinGoState> queue;
    queue.push(SimplifiedSpinGoState()); // Start with preflop
    float pot = 0.0; // Initialize pot variable
    string last_round = "preflop"; // Track the last round
    int sequence_count = 0;
    map<string, int> unique_sequences; // To store unique sequences
    bool recording_actions = false; // Flag to indicate when to start recording actions

    // Open a file to save the unique sequences
    ofstream output_file("flop_infosets.txt");
    ...
    while (!queue.empty()) {
        ...
        // Only record sequences once we reach the flop round
        if (current_state.round == "flop" && last_round != "flop") {
            pot = current_state.pot[0] + current_state.pot[1] + current_state.pot[2];
            last_round = "flop"; // Update the last round to "flop"
            recording_actions = true; // Start recording actions from this point
        }

        // Only count sequences that started in flop and are now complete
        if (recording_actions && (current_state.game_over || current_state.round != "flop")) {
            string input = current_state.action_history_string();
            string sequence = split_and_remove_last(input);
            
            // Include the pot in the sequence
            sequence += " | Pot: " + to_string(pot); // Concatenate pot information to the sequence
            
            if (unique_sequences.find(sequence) == unique_sequences.end()) {
                unique_sequences[sequence] = 1;
                sequence_count++;
                
                // Write the unique sequence to the file
                output_file << sequence << endl;
            }
            continue;
        }
    ...
    cout << "Total number of unique flop action sequences: " << sequence_count << endl;
