...
void saveflopEquitiesToCSV(const vector<pair<string, double>> &flopEquities, 
                            const string &filename = "flop_equities_processed.csv") {
...
void remove_isomorphic_hands(const string &inputFilename = "flop_equities_processed.csv", 
                             const string &outputFilename = "flop_equities_processed.csv") {
            ...
            for (int b1 = 0; b1 < static_cast<int>(RANKS.size()); b1++) {
                for (int b2 = b1; b2 < static_cast<int>(RANKS.size()); b2++) {
                    for (int b3 = b2; b3 < static_cast<int>(RANKS.size()); b3++){
                                ...
                                // Skip illegal rank distributions (more than 3 cards per rank)
                                bool valid = true;
...
// suit(r1) <= suit(r2) <= suit(r3) <= suit(r4) <= suit(r5)
void generateCanonicalSuitAssignments(
    vector<pair<string, double>> &flopEquities,
    int &count,
    int maxSamples,
    ...
    // Our fixed order for the 5 cards
    vector<int> rankIndices = { r1, r2, b1, b2, b3};  // Fixed parameter names to match function signature

    // Used cards in the deck
    vector<bool> usedCards(RANKS.size() * SUITS.size(), false);

    // partialHand will hold the 5 cards as they are assigned
    vector<Card> partialHand;
    partialHand.reserve(5);
    ...
    function<void (int)> assignSuits = [&](int cardPos) {
        // If we have assigned suits for all 5 cards, then create the canonical representation
        if (cardPos == 5) {
            string canStr = canonicalflop(partialHand);
