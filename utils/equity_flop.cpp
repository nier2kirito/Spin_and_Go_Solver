...
#include <iostream>
#include "poker_evaluator.cpp" // Evaluates a 5-card hand (hero: 2 hole + 3 board)
...
// Global constants here (We should take care of all the combinations of (2+3) cards)
const int NUM_CLUSTERS_OPP = 30;        //clusters for opponent starting-hand clustering
const int N_RANDOM_flop_SAMPLES = 1e9; // Number of flop configurations to sample
const int N_CLUSTER_RUNS = 1;          // Number of runs for k-means clustering for flops (Let's try 1 for now and see how much time it takes)

// Function prototype for parseCanonicalflop
vector<Card> parseCanonicalflop(const string &flopID, 
                                const vector<string> &RANKS, 
                                const vector<string> &SUITS);

// Add this before generateUniqueCanonicalflops
void generateCanonicalSuitAssignments(
    vector<pair<string, double>> &flopEquities, 
    int &count, 
    int maxSamples,
    const vector<string> &RANKS, 
    const vector<string> &canonicalSuits,
    int r1, int r2, int b1, int b2, int b3,
...
string canonicalflop(const vector<Card> &flop) {
    // Treat all 5 cards as a single collection
    vector<Card> allCards = flop;
...
bool isAvailableOppHand(const Card &c1, const Card &c2, const vector<Card> &heroflop) {
    return !cardInHand(c1, heroflop) && !cardInHand(c2, heroflop);
}

// Optimized function to handle all starting hand types
bool assignRepresentativeOpponentHand(int i, int j, const vector<Card> &heroflop,
    ...
    if (i == j) {
        ...
        for (size_t a = 0; a < SUITS.size(); a++) {
            for (size_t b = a + 1; b < SUITS.size(); b++) {
                ...
                if (isAvailableOppHand(c1, c2, heroflop)) {
                    oppHand = {c1, c2};
                    return true;
                }
            }
...
int evaluateflopHand(const vector<Card> &heroHole, const vector<Card> &board, PokerEvaluator &evaluator) {
    return evaluator.evaluateHand(heroHole, board);
}

vector<double> computeflopHistogram(const vector<Card> &flop, const vector<int> &oppClusters,
                                    const vector<string> &RANKS, const vector<string> &SUITS,
                                    PokerEvaluator &evaluator) {
    vector<double> histogram(NUM_CLUSTERS_OPP, 0.0);
    vector<Card> heroHole(flop.begin(), flop.begin() + 2);
    vector<Card> board(flop.begin() + 2, flop.end());
    int heroValue = evaluateflopHand(heroHole, board, evaluator);
    ...
    for (int i = 0; i < 13; i++) {
        for (int j = 0; j <= i; j++) {  // Only need to consider canonical hand types (i >= j)
            ...
            vector<Card> oppHand;
            if (!assignRepresentativeOpponentHand(i, j, flop, RANKS, SUITS, oppHand)) continue;
...
// Modified function to compute flop equity directly without opponent clustering
vector<double> computeflopEquity(const vector<Card> &flop,
                                 const vector<string> &RANKS, const vector<string> &SUITS,
                                 PokerEvaluator &evaluator) {
...
// Function to save flop equities to CSV file
void saveflopEquitiesToCSV(const vector<pair<string, double>> &flopEquities, 
                            const string &filename = "flop_equities_canonical.csv") {
    ofstream outFile(filename);
    outFile << "flopID,Equity\n";
    for (const auto &p : flopEquities) {
        outFile << "\"" << p.first << "\"," << p.second << "\n";
    }
    outFile.close();
    cout << endl << "Saved " << flopEquities.size() << " flop combinations to " << filename << endl;
}

// Function to load existing flop equities from CSV file
bool loadflopEquitiesFromCSV(vector<pair<string, double>> &flopEquities, 
                             unordered_set<string> &seenflopCanon,
                             const string &filename = "flop_equities_canonical.csv") {
    ...
    while (getline(inFile, line)) {
        ...
        // Extract flopID (enclosed in quotes)
        getline(ss, flopID, ',');
        if (flopID.size() > 2) {
            flopID = flopID.substr(1, flopID.size() - 2); // Remove quotes
        }
        ...
        flopEquities.push_back({flopID, equity});
        seenflopCanon.insert(flopID);
    }
    
    cout << "Loaded " << flopEquities.size() << " existing flop combinations from " << filename << endl;
    return true;
}

// Process a specific range of flop combinations and calculate their equities
void processflopEquitiesRange(vector<pair<string, double>> &flopEquities,
    ...
    for (int i = startLine; i < endLine && i < flopEquities.size(); i++) {
        // Parse the canonical flop representation back to card objects
        string flopID = flopEquities[i].first;
        vector<Card> flop = parseCanonicalflop(flopID, RANKS, SUITS);
        
        if (flop.size() != 5) {
            cerr << "Error: Invalid flop parsed from " << flopID << endl;
            continue;
        }
        
        // Calculate equity for this flop combination
        vector<double> equityResult = computeflopEquity(flop, RANKS, SUITS, evaluator);
        
        // Update the equity value
        flopEquities[i].second = equityResult[0];
...
// Modified function to parse a canonical flop string back to Card objects
vector<Card> parseCanonicalflop(const string &flopID, 
                                const vector<string> &RANKS, 
                                const vector<string> &SUITS) {
    vector<Card> flop;
    ...
    istringstream cardStream(flopID);
    string cardStr;
    
    while (cardStream >> cardStr && flop.size() < 5) {
        if (cardStr.length() >= 2) {
            string rank = cardStr.substr(0, cardStr.length() - 1);
            string suit = cardStr.substr(cardStr.length() - 1, 1);
            flop.push_back(Card{rank, suit});
        }
    }
...
// Function to process flop equities CSV and remove isomorphic hands
void remove_isomorphic_hands(const string &inputFilename = "flop_equities_canonical.csv", 
                             const string &outputFilename = "flop_equities_canonical.csv") {
    ...
    // Read header and all flop entries
    string header;
    ...
    unordered_map<string, vector<pair<string, double>>> rankPatternToflops;
    ...
    while (getline(inFile, line)) {
        ...
        string rankPattern = "";
        size_t pos = flopID.find("H:") + 2;
        while (pos < flopID.length()) {
            size_t end = flopID.find_first_of(" |", pos);
            if (end == string::npos) end = flopID.length();
            string card = flopID.substr(pos, end - pos);
        ...
        // Check if this flop is isomorphic to any with the same rank pattern
        bool isDuplicate = false;
        if (rankPatternToflops.find(rankPattern) != rankPatternToflops.end()) {
            for (const auto &existingflop : rankPatternToflops[rankPattern]) {
                if (are_hands_isomorphic(flopID, existingflop.first)) {
                    isDuplicate = true;
                    duplicateCount++;
                    break;
                }
        ...
        if (!isDuplicate) {
            rankPatternToflops[rankPattern].push_back({flopID, equity});
            outFile << "\"" << flopID << "\"," << equity << endl;
            uniqueCount++;
        }
        ...
        if (totalLines % 1000 == 0) {
            cout << "Processed " << totalLines << " lines. Found " << uniqueCount 
                 << " unique flops and " << duplicateCount << " duplicates.\r";
            cout.flush();
        }
    }
    
    cout << endl << "Finished processing " << totalLines << " lines." << endl;
    cout << "Found " << uniqueCount << " unique flops and " << duplicateCount << " duplicates." << endl;
    cout << "Saved unique flops to " << outputFilename << endl;
}
...
// Modified function to generate unique canonical flops with proper ordering constraints
void generateUniqueCanonicalflops(vector<pair<string, double>> &flopEquities,
                                    int maxSamples,
                                    const vector<string> &RANKS,
                                    const vector<string> &SUITS) {
    auto startTime = chrono::steady_clock::now();
    int count = flopEquities.size(); // Start counting from existing entries

    // Loop over all possible rank combinations for the 5 cards
            ...
            for (int b1 = r2; b1 < static_cast<int>(RANKS.size()); b1++) {
                for (int b2 = b1; b2 < static_cast<int>(RANKS.size()); b2++) {
                    for (int b3 = b2; b3 < static_cast<int>(RANKS.size()); b3++){
                                ...
                                if (count >= maxSamples) {
                                    cout << "\nGenerated " << count << " unique canonical flop combinations" << endl;
                                    return;
                                }
                                ...
                                // Call helper function to assign suits to the 5 cards
                                generateCanonicalSuitAssignments(flopEquities, count, maxSamples,
                                    RANKS, SUITS,
                                    r1, r2, b1, b2, b3,
...
void generateCanonicalSuitAssignments(
    ...
    vector<int> rankIndices = {r1, r2, b1, b2, b3};
    for (int i = 0; i < 5; i++) {
 ...
 // (4, 1)
rankPatternToSuitPatterns["4,1"] = {
    {"c", "d", "h", "s", "c"}
};

// (3, 2)
rankPatternToSuitPatterns["3,2"] = {
    {"c", "d", "h", "c", "d"},
            {"c", "d", "h", "c", "s"}
};

// (3, 1, 1)
rankPatternToSuitPatterns["3,1,1"] = {
    {"c", "d", "h", "c", "c"},
            {"c", "d", "h", "c", "d"},
            {"c", "d", "h", "c", "s"},
            {"c", "d", "h", "s", "c"},
...
// (2, 2, 1)
rankPatternToSuitPatterns["2,2,1"] = {
    {"c", "d", "c", "d", "c"},
            {"c", "d", "c", "d", "h"},
            {"c", "d", "c", "h", "c"},
            {"c", "d", "c", "h", "d"},
...
// (2, 1, 1, 1)
rankPatternToSuitPatterns["2,1,1,1"] = {
    {"c", "d", "c", "c", "c"},
            {"c", "d", "c", "c", "d"},
            {"c", "d", "c", "c", "h"},
            {"c", "d", "c", "d", "c"},
...
// (1, 1, 1, 1, 1)
rankPatternToSuitPatterns["1,1,1,1,1"] = {
    {"c", "c", "c", "c", "c"},
            {"c", "c", "c", "c", "d"},
            {"c", "c", "c", "d", "c"},
            {"c", "c", "c", "d", "d"},
    ...
    for (const auto& pattern : suitPatterns) {
        // Verify pattern length matches the number of cards (5)
        if (pattern.size() != 5) {
            cerr << "Error: Invalid pattern length for rank pattern: " << rankPatternKey 
                 << ", expected 5 but got " << pattern.size() << endl;
        ...
        vector<string> suitAssignment(5);
        int patternPos = 0;
        ...
        try {
            ...
            // Directly map the pattern to the 5 cards
            for (int i = 0; i < 5 && i < pattern.size(); i++) {
        ...
        string canStr;
        try {
            canStr = canonicalflop(hand);
        } catch (const std::exception& e) {
            cerr << "Exception in canonicalflop: " << e.what() 
                 << " for rank pattern: " << rankPatternKey << endl;
            continue; // Skip this pattern
        }
        ...
        if (seenCanonical.find(canStr) == seenCanonical.end()) {
            flopEquities.push_back({canStr, 0.0});
            ...
            if (count % 1000 == 0) {
                ...
                cout << "Generated " << count << " unique canonical flops. Elapsed time: " 
...
int main(int argc, char* argv[]) {
    ...
    bool dataLoaded = loadflopEquitiesFromCSV(flopEquities, seenflopCanon);
    
    // Replace the nested loops with direct canonical generation
    if (flopEquities.size() < N_RANDOM_flop_SAMPLES) {
        cout << "Generating canonical flop configurations directly..." << endl;
        cout << "Starting with " << flopEquities.size() << " already processed combinations" << endl;
        
        auto startTime = chrono::steady_clock::now();
        
        // Generate unique canonical flops directly
        generateUniqueCanonicalflops(flopEquities, N_RANDOM_flop_SAMPLES, RANKS, SUITS);
    ...
    // Save results
    saveflopEquitiesToCSV(flopEquities);
