...
// Parse a hand string (e.g., "2c 2d 2h 2s 4s")
vector<Card> parseHand(const string& handStr) {
    vector<Card> hand;
    hand.reserve(5); // Reserve space for 5 cards (2 hole + 3 board)
...
double calculateEquity2PlayersParallel(const vector<Card>& fiveCards, PokerEvaluator &evaluator, 
                                      mt19937 &randomEngine, int trials, int numThreads) {
    // Generate the remaining deck
    vector<Card> deck;
    deck.reserve(52 - fiveCards.size()); // Pre-allocate memory
    ...
    auto worker = [&](int threadTrials) {
        ...
        for (int i = 0; i < threadTrials; i++) {
            // Randomly select 3 cards from the 5 cards for the board (flop)
            vector<int> indices(5);
            iota(indices.begin(), indices.end(), 0); // Fill with 0, 1, ..., 4
            
            // Use thread-local random engine for shuffling
            shuffle(indices.begin(), indices.end(), threadRandom);
            
            vector<Card> flopCards;
            flopCards.reserve(3);
            for (int j = 0; j < 3; j++) {
                flopCards.push_back(fiveCards[indices[j]]);
            }
            ...
            mainHole.reserve(2);
            for (int j = 3; j < 5; j++) {
                mainHole.push_back(fiveCards[indices[j]]);
            }
            
            // For each trial, randomly pick turn and river cards from the deck
            int turnIdx = uniform_int_distribution<>(0, deck.size() - 1)(threadRandom);
            Card turnCard = deck[turnIdx];
            
            int riverIdx = uniform_int_distribution<>(0, deck.size() - 1)(threadRandom);
            while (riverIdx == turnIdx)
                riverIdx = uniform_int_distribution<>(0, deck.size() - 1)(threadRandom);
            Card riverCard = deck[riverIdx];
            
            // Complete the community cards with turn and river
            vector<Card> communityCards = flopCards;
...
double calculateEquity3PlayersParallel(const vector<Card>& fiveCards, PokerEvaluator &evaluator, 
                                      mt19937 &randomEngine, int trials, int numThreads) {
    // Generate the remaining deck
    vector<Card> deck;
    deck.reserve(52 - fiveCards.size()); // Pre-allocate memory
...
int main(int argc, char* argv[]) {
    ...
    while(getline(inFile, line) && lineCount < endLine) {
        ...
        string handStr, flopId;
        getline(ss, handStr, ',');
        getline(ss, flopId, ',');  // not used
        ...
        try {
            vector<Card> fiveCards = parseHand(handStr);
            if (fiveCards.size() != 5) {
                cerr << "\nWarning: Expected 5 cards, got " << fiveCards.size() << " in line: " << line << endl;
                outFile << handStr << ",ERROR,ERROR" << endl;
                continue;
            }
            
            // Pass the shared evaluator and random engine to all functions:
            double equity2p = calculateEquity2PlayersParallel(fiveCards, evaluator, randomEngine, monteCarloTrials, numThreads);
            double equity3p = calculateEquity3PlayersParallel(fiveCards, evaluator, randomEngine, monteCarloTrials, numThreads);
